...
class DetectFire3d(Node):

    def __init__(self):
        super().__init__('detect_fire_3d')

        self.get_logger().info('Detecting in 3D')

        self.ball2d_sub  = self.create_subscription(Point,"/detected_fire",self.ball_rcv_callback, 10)
        self.ball3d_pub  = self.create_publisher(Point,"/detected_fire_3d",1)
        self.ball_marker_pub  = self.create_publisher(Marker,"/fire_3d_marker",1)

        self.declare_parameter("h_fov",1.089)
        self.declare_parameter("fire_radius",0.033)
        ...
        self.fire_radius = self.get_parameter('fire_radius').get_parameter_value().double_value
    ...
    def ball_rcv_callback(self,data : Point):
        ...
        d = self.fire_radius/(math.atan(ang_size/2))
        ...
        m.pose.position.y = y
        m.pose.position.z = z
        m.scale.x = self.fire_radius*2
        m.scale.y = self.fire_radius*2
        m.scale.z = self.fire_radius*2
        m.color.r = 0.933
        m.color.g = 1.0
        m.color.b = 0.0
        m.color.a = 1.0
...
def main(args=None):
    ...
    detect_fire_3d = DetectFire3d()
    rclpy.spin(detect_fire_3d)
    detect_fire_3d.destroy_node()
    rclpy.shutdown()
